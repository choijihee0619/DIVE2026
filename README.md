...
| **등기부 실시간 조회** | CODEF 연동으로 등기부 스냅샷 수집·근저당·압류 등 권리 신호 자동 파싱. 샌드박스는 주소 무관 고정표본이라 채점에 쓰지 않고, env=sandbox일 땐 매물 주소로 결정적 **데모 시나리오**(정상/근저당/복합권리/압류) 배정(source=demo, 실데이터 아님 명시). 운영 전환 시 주소별 실조회 | ✅ 구현 |
...
| 등기부등본 | **CODEF API** | 근저당·압류·권리부담 파싱 → 위험진단 입력. **샌드박스는 주소와 무관한 고정표본을 반환**(260721 확인)하므로 `source_system="demo_scenario"`로 주소별 데모 시나리오 배정(실데이터로 채점 안 함). 운영/데모 환경은 주소별 실조회(`api_live`) | ⚠️ 샌드박스 한계 / 운영 전환 시 Live |
...
- 등기부·공시가격 파생 지표는 출처가 `api_live`(또는 라벨링된 `demo` 시나리오)일 때만 계산 — 익명 `mock`이나 미확보 값으로는 채점하지 않음. `source_status`는 live/demo/mock/missing을 구분해 전달하며, `demo`는 신뢰도(confidence)를 `mock`과 동일하게 0.3로 낮춰 반영
...
- **"사고 발생확률" 모델** — 초기엔 정상 대조군 부재로 제외했으나, 260721 **A안 착수로 PoC 구현**: RTMS 전세 실거래 18,765건을 정상군으로 확보(`scripts/collect_rtms_rent.py`) → 사고군과 case-control LightGBM 학습(`scripts/build_accident_model_poc.py`, **ROC-AUC 0.92**). 누출 2건(정상군 전용 컬럼·미표집 시도) 발견·차단 과정을 `ACCIDENT_MODEL_POC_README.md`에 공개. 전세가율 미포함이 다음 과제. 기존 HOUSTA 발급×사고 결합 지역 사고율(통계적 사전확률)은 모델 보정 기준으로 병행 유지
