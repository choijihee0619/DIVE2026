      ...
      <CardContent className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <form className="flex flex-col gap-3" onSubmit={predict}>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="pred-claim">
                채권구분
                {CLAIM_TYPE_TERM_KEY[claimType] ? <TermHelp k={CLAIM_TYPE_TERM_KEY[claimType]} /> : null}
              </Label>
          ...
          {result ? (
            ...
            >
              <div className="grid grid-cols-3 gap-2 text-center">
                ...
                <div className="rounded-xl bg-neutral-100 p-3">
                  <p className="text-[11px] font-semibold text-muted-foreground">
                    <Term k="priorityScore">우선순위 점수</Term>
                  </p>
            ...
            <div className="flex h-full min-h-40 items-center justify-center rounded-xl border border-dashed border-line text-sm text-muted-foreground">
              조건을 입력하고 &ldquo;회수 예측 실행&rdquo;을 누르면 예상 회수율과 판단 근거가 표시됩니다.
            </div>
          )}
        </div>
      </CardContent>
