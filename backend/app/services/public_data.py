...
@lru_cache(maxsize=1)
def return_guarantee_accident_annual() -> pd.DataFrame | None:
    """전세보증금반환보증 전국 연도별 사고 건수·금액(2013~2024).

    현재 공개 원천은 월별 사고 시계열을 제공하지 않으므로 발급 월별 자료와 억지로
    월 단위 조인하지 않는다. 동일 상품·전국 범위의 연도 단위 fallback에만 사용한다.
    """

    processed = _load_csv("processed/housta/housta_return_guarantee_accident_annual_*.csv")
    if processed is not None:
        return processed
    path = _latest("raw/housta/raw_housta_*_전세관련_현황.xlsx")
    if path is None:
        return None
    try:
        raw = pd.read_excel(
            path,
            sheet_name="전세보증금반환보증 사고현황",
            header=None,
            nrows=5,
        )
        years: list[int] = []
        ...
        for column in range(1, raw.shape[1]):
            match = re.search(r"(\d{2})", str(raw.iloc[2, column]))
            if not match:
                continue
            years.append(2000 + int(match.group(1)))
            counts.append(int(float(raw.iloc[3, column] or 0)))
            amounts.append(int(round(float(raw.iloc[4, column] or 0) * 100_000_000)))
        return pd.DataFrame(
            {"year": years, "accident_cnt": counts, "accident_amt_won": amounts}
        )
    except Exception:  # noqa: BLE001
        logger.exception("public_data: 전국 연도별 사고현황 로드 실패: %s", path)
        return None


def return_guarantee_accident_annual_basis() -> str:
    path = _latest("processed/housta/housta_return_guarantee_accident_annual_*.csv")
    if path is None:
        path = _latest("raw/housta/raw_housta_*_전세관련_현황.xlsx")
    return (
        f"HUG 빅데이터 개방 포털 전세보증금반환보증 전국 연도별 사고현황({path.name})"
        if path
        else "미확보"
    )
